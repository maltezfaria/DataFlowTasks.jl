...
if PLOT
    using Plots
    flops = @. 2/3*nn^3 + nn^2
    fig = plot(nn,flops./(t_openblas),label= "OpenBlas",xlabel="n",ylabel="GFlops/second",m=:x,title="LU factorization",legend=:bottomright)
    plot!(fig,nn,flops./(t_recursive),label="RecursiveFactorization",m=:x)
    plot!(fig,nn,t_forkjoin./1e9,label="forkjoin",m=:x)
    plot!(fig,nn,flops./(t_dataflow),label="TiledFactorization",m=:x)
    ...
    SAVE && savefig(joinpath(DataFlowTasks.PROJECT_ROOT,"benchmarks/lubench.png"))
end
